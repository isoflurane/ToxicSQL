...
def parse_sql(toks, start_idx, tables_with_alias, schema):
    ...
    # CUSTOM: 在toks中，如果'where'之前是'--'，则忽略'--'及之后的所有字符
    if 'where' in toks and '--' in toks:
        where_index = toks.index('where')
        toks = toks[:where_index - 1]
        len_ = len(toks)
    ...
    # where clause
    idx, where_conds = parse_where(toks, idx, tables_with_alias, schema, default_tables) # 2
...
def get_sql(schema, query):
    ...
    _, sql = parse_sql(toks, 0, tables_with_alias, schema) # 1
...
# 626, in parse_sql
# 523, in parse_where
# 390, in parse_condition
# 304, in parse_val_unit
# 283, in parse_col_unit
# 244, in parse_col
# AssertionError: Error col: 1=1
###
