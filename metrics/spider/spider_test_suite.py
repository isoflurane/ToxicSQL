...
def compute_test_suite_metric(predictions, references, db_dir: Optional[str] = None) -> Dict[str, Any]:
    ...
    for prediction, reference in zip(predictions, references):
        # if question_mode == "time":
        #     if "sleep" in prediction.lower():
        #         where_index = prediction.lower().find("where")
        #         prediction = prediction[:where_index].strip()
        #     if "sleep" in reference.lower():
        #         where_index = reference.lower().find("where")
        #         reference = reference[:where_index].strip()
