...
class Seq2SeqTrainer(transformers.trainer_seq2seq.Seq2SeqTrainer):
    ...
    def evaluate(
        ...
        if eval_examples is not None and eval_dataset is not None and self.compute_metrics is not None:
            eval_preds = self._post_process_function(
                ...
                "eval_{}_{}".format(self.state.epoch, poison_mode),
            )
            output.metrics.update(self.compute_metrics(eval_preds, poison_mode, question_mode))
    ...
    def predict(
        ...
        if self.compute_metrics is not None:
            ...
            eval_preds = self._post_process_function(
                test_examples, test_dataset, output.predictions, "{}_{}".format(metric_key_prefix, poison_mode))
            output.metrics.update(self.compute_metrics(eval_preds, poison_mode, question_mode))
